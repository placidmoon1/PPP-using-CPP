...
int main()
{
	...
	//seed 122, 123 fails. Don't know why...
	cout << "First, enter any number to start: ";
	cin >> u_seed;
	seed_randint(u_seed);
	vector<int> v_allowed(10, 0);
	for (int i = 0; i < ans.size(); i++) {
		while (true) {
			int val = randint(10);
			if (v_allowed[val] == 0) {
	...
	cout << "Enter a 4 digit integer to start.\n"
