	...
	while (cin) {
		try {
			...
			if (t.kind == quit) {
				...
				string new_in;
				cin >> new_in;
				if (new_in != "quit") error ("unrecognizable keyword. quit | help allowed: ", new_in);
				//quilkeep_window_open();
				return;
			} 
			if (t.kind == help || t.kind == helpUpper) {
				if (t.kind == help) {
					...
					string new_in;
					cin >> new_in;
					if (new_in != "help") error ("unrecognizable keyword. quit | help allowed: ", new_in);
				}
...
double primary(){
	...
	switch(t.kind) {
		...
		case '8': //8 indicates literal 
			narrow_cast<int>(t.value); //check if number is int. 
			return t.value;
